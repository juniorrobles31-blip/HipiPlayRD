...
class WIN {
	...
	public function __construct($server_time,$game_mode) {
		...
		$this->id_values=14; //Id para las jugadas ganadoras
		$this->idJDD=1; //Id juegos del dinero
		$this->dice1($server_time);
		$this->dice2($server_time);
		$this->dice3($server_time);
		$this->roulette($server_time);
	...
	private function dice1($server_time){
		...
		$NEXT_ROLL=0;
		$WonDice1=0;
		$data = $this->db->select('SHOW TABLE STATUS LIKE "gms_won_dice.1"','','');
		if($data['count']==1){
	...
	private function winners_dice1($no_play,$server_time){
		...
		if($data['count']>0){
			for ($x = 0; $x < $data['count']; $x++) {
				$id = $this->db->insert($this->tblTransaction,array("`id_user_reg`" =>$this->idJDD,"`id_user`" => $data[$x]->id_user,"`id_game`" => $data[$x]->id_game,"`nm_play`" => $no_play, "`amount`" => $data[$x]->profit, "`id_values`" => $this->id_values, "`pay`" => $data[$x]->id_trans, "`entry_date`" => $server_time),array('%i','%i','%i','%i','%s','%i','%i','%s'));
				if(empty($id)){
	...
	private function winners_dice3($no_play,$server_time){
		...
		if($data['count']>0){
			for ($x = 0; $x < $data['count']; $x++){
				...
				$id = $this->db->insert($this->tblTransaction,array("`id_user_reg`" =>$this->idJDD,"`id_user`" => $data[$x]->id_user,"`id_game`" => $data[$x]->id_game,"`nm_play`" => $no_play, "`amount`" => $profit, "`id_values`" => $this->id_values, "`pay`" => $data[$x]->id_trans, "`entry_date`" => $server_time),array('%i','%i','%i','%i','%s','%i','%i','%s'));
				if(empty($id)){
